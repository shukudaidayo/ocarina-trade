...
contract DeployOTCRegistryMainnet is Script {
    function run() external {
        ...
        vm.startBroadcast();
        OTCRegistry zone = new OTCRegistry(tokens, SEAPORT);
        vm.stopBroadcast();

        console.log("OTCRegistry deployed at:", address(zone));
    }
}

contract DeployOTCRegistryBase is Script {
    function run() external {
...
contract DeployOTCRegistryPolygon is Script {
    function run() external {
...
contract DeployOTCRegistryInk is Script {
    function run() external {
        address[] memory tokens = new address[](3);
        tokens[0] = 0x4200000000000000000000000000000000000006; // WETH
