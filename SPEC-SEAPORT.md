...
- **Chains**: Ethereum, Base, Polygon. Sepolia has a deployed testnet registry and direct Sepolia offer pages remain renderable for retained links/testing, but ocarina.trade does not expose Sepolia in the public create flow or offers browser. Ink has a deployed registry and can be used by third-party frontends, but ocarina.trade does not expose it in the public chain picker or offers browser.
...
OfferItem / ConsiderationItem {
  ...
  endAmount       // Final amount; OTCRegistry V1 requires this to equal startAmount
}
```
...
The upper 96 bits of `zoneHash` are reserved for OTCRegistry metadata. V1 packs the original consideration item count into `zoneHash` so the registry can distinguish signed consideration from fulfillment-time tips without storing a second mapping:
...
Users approve the Seaport contract directly to transfer their assets. Since OTCRegistry orders use `conduitKey: bytes32(0)`, approvals go directly to the Seaport contract address.
...
- **Current ocarina.trade site**: The exposed tip UI is an optional support-tip checkbox with a USDC amount input on chains with a configured USDC address. The site signs a `TipAuthorization` only for the selected USDC amount and the configured Ocarina support recipient. It does not currently expose custom tip recipients, native tips, or third-party frontend tips, even though the registry protocol can validate them.
...
- Protocol-level tips may be paid to the maker, the frontend operator, or any other recipient, but only if the fulfiller signs the exact tip set.
...
- **NFT data**: Alchemy NFT v3 API — `getContractsForOwner` for collection enumeration in the asset picker, `getNFTsForOwner` for fetching individual NFTs within a specific collection. For chains without Alchemy NFT API support (retained Sepolia direct links and unlisted Ink direct links), falls back to the Blockscout v2 API (`/api/v2/addresses/{addr}/nft/collections` and `/api/v2/tokens/{contract}/instances`). If `VITE_ALCHEMY_API_KEY` is not set, Alchemy-backed chains show no wallet holdings in the picker, while Blockscout-backed chains can still enumerate holdings. Users can always add assets via manual contract address / token ID entry.
...
4. **`/faq`** - FAQ page
   ...
   - Current OTCRegistry addresses list the production chains plus retained direct-link/testnet deployments, including Sepolia. Deprecated OTCZone addresses are shown separately and include Ink, because historical non-open Ink orders used that old contract.
   - No wallet connection UI on this page
...
5. **`/offers`** - Browse offers
   ...
   - Chain filter: All Chains / Ethereum / Base / Polygon. Accepts chain ID (`?chain=8453`) or name (`?chain=base`). Hidden deployments such as Sepolia and Ink are not queried by the public offers browser, though direct Sepolia offer URLs still render.
...
2. **Partial RPC fallback**: If Blockscout is unavailable, falls back to `eth_getLogs` over a recent block window only, not full history. The public offers browser scans selectable chains only, from `max(latestBlock - 49,999, deployBlock)` to `latestBlock`, chunked at 9,999 blocks on Polygon/Base and 49,999 blocks on Ethereum. Results from this path are marked `_partial`, and `/offers` shows a "Only showing recent offers. Older offers may be missing." disclaimer if any chain used the fallback. Direct reads against retained hidden deployments use the same path if invoked: Sepolia uses the 49,999-block chunk size, and Ink uses the 9,999-block chunk size.
...
- ERC-20 whitelist enforced at three layers (frontend, registration, fulfillment), with registration-time item-shape checks and ERC-165 checks for declared NFT items, including the ERC-165 invalid-interface probe (`0xffffffff`). These checks block ordinary ERC-20-as-NFT mislabeling and malformed orders, but they do not prove that an arbitrary unverified token contract is honest or fillable. Transferability policies such as ERC-5192, ERC-5484, ERC-6454, ERC721-C/ERC1155-C, criteria-specific restrictions, and maker/taker-specific restrictions are handled by frontend warnings, a static known-incompatible NFT blocklist before approval prompts, maker-side exact NFT transfer simulation at creation, and by simulating the exact Seaport fill after taker approvals but before the final fulfillment transaction. The ERC-20 whitelist is also a deployment-time security boundary: only standard, non-rebasing, non-fee-on-transfer tokens should be whitelisted, because OTCRegistry does not verify balance deltas around Seaport transfers.
- The known-incompatible NFT blocklist is intentionally conservative and non-exhaustive. Entries should be added only when there is collection-level evidence that canonical zero-conduit Seaport cannot transfer the assets, such as an ERC-5192-style soulbound implementation, a transfer-validator policy that rejects Seaport, or repeated read-only transfer simulations showing a whole-contract policy failure.
...
3. Selects chain (Ethereum / Base / Polygon) — triggers wallet network switch. If the wallet has zero native gas on the selected chain, a modal warns that gas is needed and links to Uniswap. User can dismiss with "Continue Anyway".
...
7. Clicks "Confirm" → execute screen walks through steps:
   a. Check offered assets — verifies exact maker-offered assets are still held and rejects any maker- or taker-side NFT collection on the known Seaport-incompatible blocklist before approval prompts.
   b. Approval steps — one per unique maker token contract (gas, per collection)
   c. Verify Seaport transfer — confirms Seaport approvals are visible onchain and exact maker-offered ERC-721/ERC-1155 items can be transferred by Seaport in a read-only simulation. Directed offers simulate to the directed taker; open offers use generic recipients to catch operator-level transfer-policy blocks without involving future taker approvals. Maker-side criteria items are approval-checked only because ownership and transferability depend on the token ID selected at fulfillment.
   d. Sign Seaport order — EIP-712 signature authorizing settlement (**no gas**)
   e. Sign OTCRegistry registration — EIP-712 signature binding the Seaport-derived `orderHash`, `seaportSignature`, and memo to the maker (**no gas**). The registration expires with the Seaport order's `endTime`. Separate from (d) because the Seaport signature authorizes fund transfer; this one authorizes the public listing and is what sponsored-gas relayers submit.
   f. Register order — `registerOrder` on OTCRegistry (gas, cheap)
   - Checklist recovery: for approval, signing, and registration wallet prompts, the UI can show a delayed retry affordance only while no transaction hash has been returned. Once an approval or registration transaction hash exists, the UI shows an explorer link and a "check transaction"/"check registration" action instead of blind retry, preventing accidental duplicate submissions.
...
10. UI shows a step-by-step checklist: a known-incompatible asset check, one step per token approval, a fillability simulation step, an optional cash-tip authorization signature, plus the final fulfillment action. Each step shows status (pending → checking/signing/confirming → done/failed).
11. UI checks the static known-incompatible NFT blocklist before approval prompts, walks through approval steps, simulates the exact Seaport fill from the connected wallet, asks for a gasless `TipAuthorization` signature only if the fulfiller opted into a tip, then calls `fulfillOrder` for no-tip exact-item orders or `fulfillAdvancedOrder` for criteria/tipped orders — one transaction, atomic trade
    - Checklist recovery mirrors creation: delayed retry is available for stuck approval, tip-signature, and accept wallet prompts only before a transaction hash exists. Once an approval or fill transaction hash exists, the UI shows the explorer link and "check transaction" instead of blind retry, avoiding duplicate approval/fill attempts.
...
block explorer. Sepolia is a testnet deployment for development, is not part of
the V1 production chain scope, and is intentionally hidden from the public create
flow and offers browser; the frontend still retains its constants so FAQ links
and direct Sepolia offer pages render. Ink has a live OTCRegistry deployment,
but ocarina.trade intentionally leaves it out of the public chain picker and
offers browser to keep the site focused on chains with meaningful NFT trading
activity. That UI decision does not deprecate the Ink registry itself: other
frontends may use the Ink deployment directly, and direct historical links can
still resolve against the retained constants.
...
- Publish ERC-7730 descriptors for the current OTCRegistry deployments so supported wallets can show human-readable signing prompts. Draft descriptors and reference tests live under `erc7730/ocarina-trade/` before upstream submission.
- Primary targets: `registerOrder(OrderRegistration)` calldata ("Publish offer"), the OTCRegistry EIP-712 registration signature ("Authorize offer"), and the optional `TipAuthorization` EIP-712 signature ("Authorize tip"). The descriptors should bind `name: "OTCRegistry"`, `version: "1"`, and the current deployment set: Ethereum, Sepolia, Base, Polygon, and the unlisted Ink deployment.
- `registerOrder` calldata metadata should display the maker, memo when present, offered items, requested consideration, expiration, and the packed `zoneHash`. The descriptor should require `orderType == FULL_RESTRICTED` and `conduitKey == bytes32(0)` as hidden invariants because both are contract / site requirements and not useful wallet-facing fields. Original consideration item recipients can also be hidden because registration requires every recipient to equal the maker, which is already implied by the "Maker receives" label. Generic ERC-7730 formatting cannot decode OTCRegistry's packed taker/count/version bitfield from `zoneHash`, so the descriptor should not pretend to display the taker address unless wallet tooling later supports that transform.
- Include descriptor tests with a sample `registerOrder` transaction and sample EIP-712 registration / tip typed data, then submit to the Ethereum ERC-7730 registry: `https://github.com/ethereum/clear-signing-erc7730-registry`.
- Do not try to own generic Seaport signing metadata. Seaport order signatures and `fulfillOrder` calldata target the canonical Seaport contract, so OTCRegistry-specific rendering there depends on wallet support for contextual constraints such as `zone == OTCRegistry`.
